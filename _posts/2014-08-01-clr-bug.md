...
We decide to look at where this member is supposed to be invoked. It turns out to be in `System.Linq.Expressions.Interpreter.QuoteInstruction.ExpressionQuoter`, specifically the `VisitParameter` method. I've copied decompiled it with ILSpy, anda again highlighted the offending attempt to access the `Value` property.
...
We basically know how to do what we want in IL, but we spend a lot of time compiling sample applications to get the namespaces and assemblies right, so that we can actually write the newly-hacked IL code. We also spend some time fixing errors in the IL code that `ilasm` reports to us.

At some point, though, we settle on the following IL code, and ultimately **swap out the lines above for these lines**. (NB, the IL addresses are optional.)
...
Finally, we re-generate the DLL from this IL with the following command:
...
We re-decompile this with ILSpy to make sure that the everything looks like it should (notice the highlighted part points at a call to `Property` instead):
...
So, finally, we recompile to fetch our original project, so that it fetches the new binary from the GAC (i.e., the Global Applications Cache).

Then we run our code. Magically, it now works.

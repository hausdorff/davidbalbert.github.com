---
layout: post
title: "The time I found a CLR bug and fixed it by hand-patching the DLL."
...
In either case, I found the error to be completely baffling. I showed my awesome colleague [Bart De Smet](http://www.amazon.com/5-0-Unleashed-Bart-De-Smet/dp/0672336901) and he too was puzzled.
...
We decide to look at where this member is supposed to be invoked. It turns out to be in `System.Linq.Expressions.Interpreter.QuoteInstruction.ExpressionQuoter`, specifically the `VisitParameter` method. I've copied it again, highlighting the offending attempt to access the `Value` property for your benefit.

<center><img src="images/expressionquoter.png" width=1200/></center>

Look at that! It's clearly trying to access a **field** called `Value`, but we know from the last code snippet that `Value` is unambiguously a **property**. So this code is completely wrong, and it's been wrong since about 2008.

Further proof this has been a bug for a long time is that the `Value` property actually comes from the `IStrongBox` interface &mdash; and since interfaces can't have fields at all, we know that `Value` has always been a property.
...
So, of course, that's what we did, right? HAHAHAHAHA of course not. After Bart finished grumbling and accepted that we were not leaving until we resolved the issue, we sat down and **we fucking patched the binary by hand.** Fuck the police.

We start by grabbing `System.Linq.Expressions.dll` (the DLL where all this code is) and decompiling it to IL using this command:

```
ildasm /out=System.Linq.Expressions.il System.Linq.Expressions.dll
```

This produces a big-ass IL file. We open it and ctrl-F for the string `"Value"` (with quotes), which should lead us to the code that does this awful `Expression.Field` thing we saw above.

This leads us to line # 88,200:
...
*(Bart grumbles and moans some more about this like a sane person should, but this is no time to be sane. This is a time to fix the problem.)*

After a bit of trial and error, we replace this code with the following code (IL addresses are optional):
...
Finally, we re-generate the binary from this IL with the following command:
...
We re-decompile this with ILSpy to make sure that the everything looks like it should (notice the highlighted part points at the property instead):
...
So, finally, we recompile to fetch this new binary from the GAC (i.e., the Global Applications Cache), and run our code.

It now works.

We follow this up by sending the world's best bug report to the .NET native team.
